...
export function generateSearchMoreDialog(
  ...
  return `<dialog id="${dialogId}" label="More Search Fields" mboname="${mboName}">
...
\t\t\t\t<pushbutton default="true" id="${generateId()}" label="Search" mxevent="dialogok"/>
\t\t\t\t<pushbutton id="${generateId()}" label="Cancel" mxevent="dialogcancel"/>
\t\t\t</buttongroup>
\t\t</dialog>`;
}
