 ...
 * Fields are grouped by: area, tabName, and relationship (for detail fields)
 */
function getGroupKey(field: SAFieldDefinition): string {
  ...
  if (field.area === 'detail') {
    return `${field.area}:${field.tabName || 'Main'}:${field.relationship || 'default'}`;
  }
  return `${field.area}:${field.tabName || 'Main'}`;
}
...
export function moveFieldUp(
  ...
  // Sort by order to find position within group
  const sortedGroup = [...groupFields].sort((a, b) => a.field.order - b.field.order);
  ...
  const targetOrder = newFields[index].order;
  const aboveOrder = newFields[fieldAbove.originalIndex].order;
...
export function moveFieldDown(
  ...
  const newFields = [...fields];
  const targetOrder = newFields[index].order;
  const belowOrder = newFields[fieldBelow.originalIndex].order;
...
export function normalizeFieldOrders(
  ...
  groups.forEach((groupFields) => {
    ...
    const sorted = [...groupFields].sort((a, b) => a.field.order - b.field.order);
